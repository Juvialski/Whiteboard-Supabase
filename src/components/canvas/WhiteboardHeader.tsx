              ...
              {canManage ? (
                <>
                  <div className={`mt-2 mx-1 px-2.5 py-2 rounded-xl text-[10px] font-semibold flex items-center gap-2 ${
                    ...
                    <span>
                      {studentsCanWrite
                        ? "Global writing is on. You can make any individual student view only below."
                        : "Global student lock is on. Individual editors will remain locked until it is turned off."}
                    </span>
                  </div>
                  ...
                  <div className="px-2 pt-3 pb-1 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => onRefreshBoardMembers && void onRefreshBoardMembers()}
                      disabled={isBoardMembersLoading}
                      className="normal-case tracking-normal text-[9px] font-bold text-blue-600 hover:text-blue-700 disabled:text-slate-300"
                    >
                      Refresh
                    </button>
                  </div>

                  {isBoardMembersLoading && boardMembers.length === 0 ? (
                    <div className="px-2 py-4 flex items-center justify-center gap-2 text-xs text-slate-400">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      Loading members...
                    </div>
                  ) : boardMembers.length === 0 ? (
                    <p className="px-2 py-3 text-xs text-slate-400">No redeemed student memberships yet.</p>
                  ) : (
                    <div className="space-y-1">
                      {boardMembers.map((member) => {
                        const collab: any = collaborators.find((entry: any) => entry.id === member.userId);
                        const isOnline = Boolean(collab);
                        const isFollowed = followedUserId === member.userId;
                        const expiresAt = member.expiresAt ? Date.parse(member.expiresAt) : NaN;
                        const isExpired = Number.isFinite(expiresAt) && expiresAt <= Date.now();
                        const isUpdating = memberUpdatingId === member.userId;
                        const displayName = collab?.name || member.displayName || "Guest user";
                        const roleLabel = isExpired
                          ? "Expired"
                          : member.role === "viewer"
                            ? "View only"
                            : studentsCanWrite
                              ? "Can edit"
                              : "Editor · globally locked";

                        return (
                          <div key={member.userId} className="rounded-xl border border-slate-100 bg-white px-2.5 py-2 shadow-sm">
                            <div className="flex items-center gap-2">
                              <span
                                className={`w-3 h-3 rounded-full shrink-0 border-2 border-white ring-1 ${isOnline ? "ring-emerald-300" : "ring-slate-200"}`}
                                style={{ backgroundColor: collab?.color || "#cbd5e1" }}
                                title={isOnline ? "Online" : "Offline"}
                              />
                              <span className="min-w-0 flex-1 truncate text-xs font-semibold text-slate-700" title={displayName}>{displayName}</span>
                            ...
                            <div className="mt-2 flex items-center justify-end gap-1.5">
                              {isOnline && (
                                <button
                                  type="button"
                                  onClick={() => setFollowedUserId(isFollowed ? null : member.userId)}
                  ...
                  {collaborators.filter((collab: any) => !boardMembers.some((member) => member.userId === collab.id)).length > 0 && (
                    <div className="mt-2">
                      <div className="px-2 py-1 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Other connected users</div>
                      {collaborators
                        .filter((collab: any) => !boardMembers.some((member) => member.userId === collab.id))
                        .map((collab: any) => {
                          ...
                          return (
                            <button
                              ...
                              className="w-full px-2 py-2 rounded-xl flex items-center gap-2 text-left hover:bg-slate-50 text-slate-700"
                            >
