...
export default function Dashboard({ 
  ...
  const handleOpenRenameModal = (board: Whiteboard, e: React.MouseEvent) => {
    e.stopPropagation();
    setBoardToRename({
      id: board.id,
      name: board.name || '',
  ...
  const handleRenameBoardSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!boardToRename || !boardToRename.name.trim()) return;

    const targetId = boardToRename.id;
    const trimmedName = boardToRename.name.trim();
    const trimmedDesc = (boardToRename.description || '').trim();

    setIsRenaming(true);
    try {
      if (!isSandboxEnvironment()) {
        const { doc, setDoc } = await import('../lib/supabaseDb');
        await retryAsync(() => setDoc(doc(db, 'whiteboards', targetId), {
          name: trimmedName,
        ...
        const currentBoards = getSandboxLocalBoards();
        const updated = currentBoards.map((b: any) =>
          b.id === targetId ? { ...b, name: trimmedName, description: trimmedDesc, updatedAt: Date.now() } : b
        );
      ...
      alert(err instanceof Error ? err.message : "Failed to rename board.");
    } finally {
      setIsRenaming(false);
    }
  };
    ...
    <div className="h-full bg-slate-50 text-slate-800 flex flex-col font-sans overflow-y-auto" id="lucid-dashboard">
      ...
      {boardToRename && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center space-x-2 text-indigo-600">
                <Pencil className="w-5 h-5" />
                <h3 className="text-lg font-bold text-slate-900">Rename Whiteboard</h3>
              </div>
              <button
            ...
            <form onSubmit={handleRenameBoardSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={boardToRename.name}
                  onChange={(e) => setBoardToRename({ ...boardToRename, name: e.target.value })}
                  placeholder="e.g. Math Lesson 4"
                  required
                  maxLength={160}
                  autoFocus
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white text-slate-800"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  value={boardToRename.description}
                  onChange={(e) => setBoardToRename({ ...boardToRename, description: e.target.value })}
                  placeholder="Brief notes or description..."
                  maxLength={1000}
                  rows={2}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white text-slate-800 resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={isRenaming || !boardToRename.name.trim()}
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold text-sm rounded-xl transition-all shadow-sm flex items-center gap-2 disabled:opacity-50 cursor-pointer"
                >
                  {isRenaming ? (
                    <>
